...
public class GitCommissioningUtils {
    ...
    public static void loadConfiguration() {
        ...
        List<GitConfigRecord> configRecordList = context.getPersistenceInterface()
                .query(new SQuery<>(GitConfigRecord.META));
        ...
        try {
            if (ignitionConf.exists() && ignitionConf.isFile()) {
                ...
                if (projectManager.getProjectNames().contains(config.getIgnitionProjectName())) {
                    logger.info("The configuration of the git module was interrupted because the project '"
                            + config.getIgnitionProjectName() + "' already exist.");
                    return;
                }
                ...
                if (config.getRepoURI() == null || config.getRepoBranch() == null
                        ...
                        || (config.getUserPassword() == null && config.getSshKey() == null)
                        || config.getUserEmail() == null) {
                    throw new RuntimeException("Incomplete git configuration file.");
                }

                projectManager.createProject(config.getIgnitionProjectName(),
                        new ProjectManifest(config.getIgnitionProjectName(), "", false, false, ""), new ArrayList<>());
                ...
                if (persistenceInterface.queryOne(query) != null) {
                    logger.info(
                            "The configuration of the git module was interrupted because the GitProjectsConfigRecord '"
                                    + config.getIgnitionProjectName() + "' already exist.");
                    return;
                }
                GitProjectsConfigRecord projectsConfigRecord = persistenceInterface
                ...
                if (userSecretFilePath != null) {
                    config.setSecretFromFilePath(Paths.get(userSecretFilePath),
            ...
            } else if (!configRecordList.isEmpty()) {
                config = parseGitConfig(configRecordList);
    ...
    protected static GitCommissioningConfig parseConfigLines(byte[] ignitionConf) {
        ...
        try (BufferedReader reader = new BufferedReader(
                new InputStreamReader(new ByteArrayInputStream(ignitionConf), StandardCharsets.UTF_8))) {
            String line;
    ...
    protected static GitCommissioningConfig parseGitConfig(List<GitConfigRecord> gitConfigRecords) {

        GitCommissioningConfig config = new GitCommissioningConfig();
        for (GitConfigRecord gcr : gitConfigRecords) {
            switch (gcr.getKey()) {
                case "repo.uri":
                    config.setRepoURI(gcr.getValue());
                    break;
                case "repo.branch":
                    config.setRepoBranch(gcr.getValue());
                    break;
                case "ignition.project.name":
                    config.setIgnitionProjectName(gcr.getValue());
                    break;
                case "ignition.user.name":
                    config.setIgnitionUserName(gcr.getValue());
                    break;
                case "user.name":
                    config.setUserName(gcr.getValue());
                    break;
                case "user.password":
                    config.setUserPassword(gcr.getValue());
                    break;
                case "user.email":
                    config.setUserEmail(gcr.getValue());
                    break;
                case "user.ssh.key.file.path":
                    config.setSshKey(gcr.getValue());
                    break;
                case "commissioning.import.tags":
                    if (gcr.getValue().equals("true"))
                        config.setImportTags(true);
                    else
                        config.setImportTags(false);
                    break;
                case "commissioning.import.themes":
                    if (gcr.getValue().equals("true"))
                        config.setImportThemes(true);
                    else
                        config.setImportTags(false);
                    break;
                case "commissioning.import.images":
                    if (gcr.getValue().equals("true"))
                        config.setImportImages(true);
                    else
                        config.setImportImages(false);
                    break;
                case "init.defaultBranch":
                    config.setInitDefaultBranch(gcr.getValue());
                    break;
            }
        }
